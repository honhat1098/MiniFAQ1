...
export const StudentView: React.FC<StudentViewProps> = ({ gameState, localPlayerId, setLocalPlayerId }) => {
  ...
  if (gameState.phase === GamePhase.PLAYING || gameState.phase === GamePhase.RESULT_REVEAL || gameState.phase === GamePhase.LEADERBOARD || gameState.phase === GamePhase.FINISHED) {
      ...
      return (
          <div className="flex flex-col max-w-lg mx-auto bg-black/20 md:rounded-3xl overflow-hidden md:border border-white/10 md:my-4 h-[100dvh] md:h-[90vh]">
              {/* Header */}
              <div className="p-4 bg-brand-dark/80 backdrop-blur-md flex items-center gap-3 border-b border-white/10 shrink-0">
              ...
              {/* Input Area (Options) */}
              {gameState.phase === GamePhase.PLAYING && !hasAnswered && (
                  <div className="p-3 bg-brand-dark/90 backdrop-blur-md border-t border-white/10 grid grid-cols-1 gap-2 shrink-0 pb-safe">
              ...
              {gameState.phase === GamePhase.PLAYING && hasAnswered && (
                   <div className="p-6 text-center opacity-60 text-sm animate-pulse shrink-0">
